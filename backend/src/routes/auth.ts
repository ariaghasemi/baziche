...
import { newOpaqueToken, signAccessToken, verifyAccessToken } from '../lib/jwt';
...
authRoutes.post('/challenge', async (c) => {
  ...
  const row = await c.env.DB.prepare('SELECT salt, iterations FROM users WHERE phone = ?')
    .bind(phone)
...
authRoutes.post('/register', async (c) => {
  ...
  const exists = await c.env.DB.prepare('SELECT phone, username_lower AS u FROM users WHERE phone = ? OR username_lower = ?')
  ...
  await audit(c.env.DB, 'user.register', id, { phone: phone.slice(0, 6) + '***' });
  return c.json({
    success: true,
...
authRoutes.post('/login', async (c) => {
  ...
  const rl = await checkLoginRateLimit(c.env.DB, phone, String(ip));
  if (!rl.allowed) {
  ...
  const now = nowSec();
  await c.env.DB.prepare('UPDATE users SET last_login = ?, updated_at = ? WHERE id = ?').bind(now, now, row.id).run();
  // Successful login resets the brute-force window for this phone.
  await c.env.DB.prepare('DELETE FROM login_attempts WHERE phone = ?').bind(phone).run();
  const session = await issueSession(c.env, row.id, b.device ?? null);
  await audit(c.env.DB, 'user.login', row.id, {});
  void pruneLoginAttempts(c.env.DB).catch(() => {});
...
async function mePayload(db: D1Database, userId: string) {
  ...
  const pc = await db.prepare("SELECT COUNT(*) AS n FROM projects WHERE user_id = ? AND status = 'active'")
    .bind(userId)
    .first<{ n: number }>();
  return {
...
meRoutes.get('/me', requireAuth, async (c) =>
  c.json({ success: true, ...(await mePayload(c.env.DB, c.get('userId'))) }),
);
