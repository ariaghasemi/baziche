...
// Per-kind allowlist + size caps (MVP). Magic-byte verification lands in Phase 2
// (requires reading bytes server-side; presigned direct upload keeps MVP free-tier safe).
...
assetRoutes.post('/presign', async (c) => {
  ...
  const p = await c.env.DB.prepare('SELECT id, user_id AS uid, shard_id AS shardId FROM projects WHERE id = ?')
    .bind(b.projectId)
  ...
  const shard = await storageRouter.resolveShard(c.env.DB, 'asset');
...
assetRoutes.post('/commit', async (c) => {
  ...
  const p = await c.env.DB.prepare('SELECT id, user_id AS uid FROM projects WHERE id = ?')
    .bind(b.projectId)
    .first<{ id: string; uid: string }>();
  if (!p || p.uid !== userId) throw err('PROJECT_NOT_FOUND', 'Project not found', 404);
  if (!b.key.startsWith('assets/') || b.key.includes('..')) throw err('VALIDATION_ERROR', 'Bad key', 400);

  const head = await c.env.R2_ASSETS.head(b.key);
  ...
  await c.env.DB.prepare(
    ...
    .run();
  await audit(c.env.DB, 'asset.commit', userId, { assetId: id, kind: b.kind, bytes: b.bytes });
