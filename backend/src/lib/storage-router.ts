...
export const storageRouter: IStorageRouter = {
  async resolveShard(
    ...
    const cands = (rows.results ?? []).filter(
      (s) => s.health !== 'down',
    );

    if (cands.length === 0) {
      throw new Error(`no healthy storage shard for ${kind}`);
    }
...
export async function touchShard(
  db: D1Database,
  shardId: string,
  ok: boolean,
  latencyMs: number,
  ...
  try {
    await db
      .prepare(
        `UPDATE storage_shards SET
           last_checked = ?,
           error_rate = CASE
             WHEN ? THEN error_rate * 0.9
             ELSE MIN(1, error_rate * 0.9 + 0.1)
           END,
           latency_ms = ?,
           health = CASE
             WHEN ? THEN health
